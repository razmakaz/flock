...
import { setLangCookie } from "$lib/server/cookies";
...
export const handle: Handle = async ({ event, resolve }) => {
	...
	if (!lang) {
		lang = event.request.headers.get("Accept-Language")?.substring(0, 2) ||
			"en";
	}
	event.locals.lang = lang;
	setLangCookie(event.cookies, lang);
	...
	if (!SUPPORTED_LANGUAGES.includes(langFromUrl)) {
		...
		return Response.redirect(
			new URL(`/${lang}${event.url.pathname}`, event.url),
			307,
		);
	} else if (
		langFromUrl !== event.locals.lang &&
		!event.url.pathname.includes("/lang")
	) {
		...
		return Response.redirect(
			new URL(
				`/${event.locals.lang}${
					event.url.pathname.slice(langFromUrl.length + 1)
				}`,
				event.url,
			),
			307,
